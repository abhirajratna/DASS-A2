...
class Game:
    ...
    def trade(self, seller, buyer, prop, cash_amount):
        ...
        """
        if cash_amount <= 0:
            print("  Trade failed: cash amount must be positive.")
            return False
    ...
    def auction_property(self, prop):
        ...
        if prop.owner is not None:
            print(f"  {prop.name} is already owned by {prop.owner.name}. Auction cancelled.")
            return
    ...
    def _apply_card(self, player, card):
        ...
        if action is None:
            print("  Card has no action. Ignoring.")
            return
        ...
        if action == "collect":
            try:
                ...
                amount = 0
                print("  Bank cannot pay card amount right now; payout treated as $0.")
